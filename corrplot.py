...
def main(plot_data, pheno, fields,se_fields, x_title, y_title, output_name, pval_field=None, p_threshold=None, exp_betas=False):
    ...
    plot=(ggplot(data=plot_data,mapping=aes(x=x_title,y=y_title))+
        ...
        annotate("text",label="R^2 (pearsonr): {:>5.2g} slope: {:>5.2g} stderr: {:>5.2g}".format(r_2_normal,reg.slope,reg.stderr),x=xlim[0]+(xlim[1]-xlim[0])*0.5,y=ylim[0]+(ylim[1]-ylim[0])*0.99,size=10 )+
        annotate("text",label="R^2 (weighted): {:>5.2g} slope: {:>5.2g} stderr: {:>5.2g}".format(r_2_weighted,reg_w.slope,reg_w.stderr),x=xlim[0]+(xlim[1]-xlim[0])*0.5,y=ylim[0]+(ylim[1]-ylim[0])*0.94,size=10 )+
